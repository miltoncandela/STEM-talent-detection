...
def filter_csv(metric, n_features=100):
    ...
    x = pd.read_csv('processed/combined_df_3.csv', dtype={'MCE_Category': 'category', 'PSI_Category': 'category',
                                                          'ID': 'string', 'Take': int})
    ...
    x = x.drop([column for column in x.columns if pearsonr(x.loc[:, column], y)[1] > 0.05], axis=1)
